...
from islaMcts.test_data import TestData
...
def run(agent_type: str, params: MctsParameters | PwParameters | DpwParameters, noise: bool, number: int,
    ...
    np.savetxt(fname=f"../output/state_log_{agent_type}_{number}{'_Noise' if noise else ''}.csv", X=state_log,
               delimiter=",")
    extra_log = np.array(extra_log)
    np.savetxt(fname=f"../output/extra_log_{agent_type}_{number}{'_Noise' if noise else ''}.csv", X=extra_log,
               delimiter=",")
...
def run_test_instance(testData: TestData):
    ...
    run(testData.agent_type, testData.param, testData.noise, run_test_instance.counter, real_env, sim_env)
...
if __name__ == '__main__':
    ...
    tests = [
        ...
        #         action_selection_fn=ucb1,
        #         gamma=1,
        #         rollout_selection_fn=discrete_default_policy(11),
        #         state_variable="_state",
        #         max_depth=500,
        #         n_actions=11
        #     ),
